...
.footnote[   
`r kableExtra::text_spec("Extra exercise 9.", background = "purple", color = "white", bold = TRUE)` What is the object translation for the list and dataframe?
]
--
...
`r emo::ji("snake")` In Python `np` methods are applied by following the object with a dot: 
...
`r emo::ji("blue heart")` In R we access and apply `np` methods in a R like way: 
...
From now on  
`r kableExtra::text_spec("=> Run Chunk", background = "lavender", bold = TRUE, color = "black")`  
...
From now on  
`r kableExtra::text_spec("=> Run added line", background = "lavender", bold = TRUE, color = "black")`  
