...
```
````
I will add a prominent comment to my code blocks indicating which programming langauge is used in that chunk.

--

You may also want to add some text such as "R packages needed"

--

We can add any other R packages we need later
...
Again, we will add any other Python modules we need later.
...
To use `os` methods interactively in later Python code chunks we need to run this code chunk.

Use the green arrow on the right of the chunk to run it.

![python mods code chunk with the run code green arrow on the right indicated](pics/run_chunk.png)
...
If you have successfuly imported the `os` module you can use it like this:
...
We will read one of the spectrum files to learn how to access dataframes in both sessions.

--

You'll need to add the `readxl`<sup>1</sup> package to your list of library statements in the R chunk called pkgs
...
violets are `r kableExtra::text_spec("blue", background = "lightblue")`
