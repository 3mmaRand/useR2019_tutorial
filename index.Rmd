...
In Python you might<sup>1</sup> use the [NumPy package](http://www.numpy.org/)<sup>2</sup>. NumPy arrays behave like R vectors/arrays.

.footnote[
[1]Python doesn't have a native array data structure but it has the list.  
[2] NumPy is the fundamental package for scientific computing with Python. It is part of the 
SciPy ecosystem.  
...
The Python code looks like this (don't type it):
```{python eval=FALSE}
import numpy as np
...
For this section on using `repl_python()` I will show the prompt in the code.
...
```{r eval=FALSE}
>  repl_python() 
Python 3.6.5 (C:\\PROGRA~3\\ANACON~1\\python.exe)
...
`r kableExtra::text_spec("Extra exercises", background = "darkturquoise", color = "black")`
