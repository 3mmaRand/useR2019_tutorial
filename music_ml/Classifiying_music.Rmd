...
Let's read one of the spectrum files in order to investigate passing dataframes between sessions. We can use the `readxl`<sup>1</sup> package. I have add that to the R chunk named `pkgs` 
...
sd_2 <- sd(spectrum$`1`)
```
The first feature has a $\bar{x} \pm s.d.$ of `r round(mean_2,2 )` $\pm$ `r round(sd_2, 2)`.
...
This will be carried out with Python. The separate _segments.xlsx files are read in to a single Pandas dataframe, `df_seg` This requires a nested for loop to iterate through the directories and through the files in the directories. The information about the segments in _SegmentInfo.xlsx files are similarly read into a Pandas dataframe, `df_info` and a column is added to capture the instrument labeling.
